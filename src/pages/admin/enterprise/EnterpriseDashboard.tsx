      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-slate-900 border border-slate-800 p-5 rounded-2xl">
          <div>
            <h1 className="text-xl font-extrabold text-white flex items-center gap-2">
              ...
              <Badge className="bg-orange-500/10 border border-orange-500/30 text-orange-400">LIVE SYNC</Badge>
            </h1>
          ...
          <button
            ...
            className="inline-flex items-center gap-2 px-4 py-2 bg-slate-800 hover:bg-slate-700 text-slate-200 rounded-xl text-xs font-bold transition border border-slate-700 self-start sm:self-auto"
          >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4">
          {METRIC_CARDS.map((card, i) => {
            ...
            return (
              <div key={i} className="bg-slate-900 border border-slate-800/80 p-4 rounded-2xl space-y-3">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 bg-slate-900 border border-slate-800 rounded-2xl p-5 space-y-4">
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs font-semibold text-slate-400">{recentOrders.length} Recent Transactions</span>
            </div>
                  ...
                  {recentOrders.length === 0 ? (
                    ...
                    recentOrders.map((order) => (
                      <tr key={order.id} className="hover:bg-slate-800/50 transition">
                        <td className="p-3 font-mono text-orange-400 font-bold">#{order.id.slice(0, 8)}</td>
                        <td className="p-3">{order.customerName || order.userId || "Marketplace Buyer"}</td>
                        <td className="p-3 font-extrabold text-white">৳{(order.totalAmount || order.price || 0).toLocaleString("en-BD")}</td>
                        <td className="p-3">
            ...
            <div className="space-y-3">
              <div className="p-3 rounded-xl bg-slate-950 border border-slate-800/80 flex items-center justify-between">
                <div className="flex items-center gap-2 text-xs">
                  ...
                  <span className="text-slate-300 font-medium">Firestore Realtime Stream</span>
                </div>
                <Badge className="bg-emerald-500/10 text-emerald-400 text-[10px]">ONLINE</Badge>
              </div>

              <div className="p-3 rounded-xl bg-slate-950 border border-slate-800/80 flex items-center justify-between">
                <div className="flex items-center gap-2 text-xs">
                  <div className="h-2 w-2 rounded-full bg-emerald-400" />
                  <span className="text-slate-300 font-medium">Supplier API Endpoint Proxy</span>
                </div>
              ...
              <div className="p-3 rounded-xl bg-slate-950 border border-slate-800/80 flex items-center justify-between">
                <div className="flex items-center gap-2 text-xs">
                  <div className="h-2 w-2 rounded-full bg-emerald-400" />
                  <span className="text-slate-300 font-medium">Brute Force Guard (Lockout)</span>
                </div>
                <Badge className="bg-emerald-500/10 text-emerald-400 text-[10px]">PROTECTED</Badge>
              </div>

              <div className="p-3 rounded-xl bg-slate-950 border border-slate-800/80 flex items-center justify-between">
                <div className="flex items-center gap-2 text-xs">
                  <div className="h-2 w-2 rounded-full bg-orange-400" />
                  <span className="text-slate-300 font-medium">15-Min Inactivity Auto Logout</span>
                </div>
                <Badge className="bg-orange-500/10 text-orange-400 text-[10px]">ENABLED</Badge>
              </div>
            </div>
          </div>
        </div>
